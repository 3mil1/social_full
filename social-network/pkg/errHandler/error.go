...
func SystemError(err error) *AppError {
	return NewAppError(err, "internal system errHandler", http.StatusInternalServerError)
}
...
func Unauthorized(err error) *AppError {
	return NewAppError(err, "unauthorized", http.StatusUnauthorized)
}

func LoginAgain(err error) *AppError {
	return NewAppError(err, "please login again", http.StatusUnauthorized)
}
